...
class EmbeddedDrawerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.setTitle(R.string.s_drawer_item_embedded_drawer)
        //set the back arrow in the toolbar
        supportActionBar?.setDisplayHomeAsUpEnabled(true)

        // Create a few sample profile
        // NOTE you have to define the loader logic too. See the CustomApplication for more details
        val profile = ProfileDrawerItem().withName("Sentenz").withEmail("info@sentenz.eu").withIcon("https://avatars3.githubusercontent.com/u/1476232?v=3&s=460")
        ...
        slider.apply {
            ...
            itemAdapter.add(
                    ...
                    PrimaryDrawerItem().withName(R.string.s_drawer_item_action_bar_drawer).withIcon(FontAwesome.Icon.faw_home).withBadge("22").withBadgeStyle(BadgeStyle(Color.RED, Color.RED)).withIdentifier(2),
            ...
            onDrawerItemClickListener = { v, drawerItem, position ->
                if (drawerItem is Nameable) {
                    MaterialDialog(this@EmbeddedDrawerActivity).show {
                        title(text = drawerItem.name?.getText(this@EmbeddedDrawerActivity))
                        message(text = "Position " + position.toString())
